...
```{r}
colrs <- c("lightgreen", "tomato", "gold",'lightBlue')
V(g)$color <- colrs[VERTEX$major]
E(g)$arrow.size <- .5

count_arrowsTo <- EDGE %>% count(to)
vertex.size <- c()

# find the count for all in-arrows 
for(target in VERTEX$id){
  vertex.notFound <- TRUE
  for(target.id in c(1:length(count_arrowsTo$to))){
    vertex.id <- as.character(count_arrowsTo$to[target.id])
    if (target == vertex.id) {
      vertex.count <- as.numeric(count_arrowsTo$n[target.id])
      vertex.size <- append(vertex.size, vertex.count)
      vertex.notFound <- FALSE
    }
  }
  if (vertex.notFound) {
    vertex.size <- append(vertex.size, 0)
  }
}

V(g)$size <- (vertex.size + 5)*3
par(mar=c(0,0,0,0))
plot(g,layout=layout.fruchterman.reingold)
...
C2 %>% gather(labelssss,classA,2:7,na.rm = TRUE,convert = FALSE)
C3$count <- 1
C3<- filter(C3,class != "")
C3 <- unique(C3)
...
plot(g,layout = layout.fruchterman.reingold, vertex.size = 7, vertex.label.cex = 0.8, vertex.label.color = "black", vertex.color = 'orange')
...
According to the graph, Guoliang Xu has the highest degree of centrality, which means he has the most connections. Yifei Zhang has the highest betweenness, which means he appears on most of the shortest path.
...
Accoring to the graph, there are two major groups/clusters and 11 smaller clusters. Yifei Zhang is in the middle of this two major groups.
